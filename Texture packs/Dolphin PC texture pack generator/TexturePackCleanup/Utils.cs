...
namespace TexturePackCleanup
{
  public static class Utils
  {
    private static Dictionary<string, string[]> _GetDteTextureMap()
    {
      ...
      return dteTextureMap ?? throw new NullReferenceException(nameof(dteTextureMap) + " is null.");
    }

    private static Dictionary<string, string[]>? _dteTextureMap = null;

    public static Dictionary<string, string[]> DteTextureMap => _dteTextureMap ??= _GetDteTextureMap();

    // Checks whether a texture is used on GameCube/Wii
    public static bool IsUsedOnDolphin(string textureName, bool isDolphinHashedName, out string unusedReason)
    {
      if (isDolphinHashedName)
      {
        if (!DteTextureMap.Values.SelectMany(x => x).Contains(textureName))
        {
          unusedReason = "it does not exist in the GameCube version's textures archive";
          return false;
        };
        if (UnusedTexturesDolphin
          .SelectMany(unusedTexture => DteTextureMap[unusedTexture])
          .Contains(textureName))
        {
          unusedReason = "it's known to be unused on GameCube & Wii";
          return false;
        };
      }
      else
      {
        if (UnusedTexturesDolphin.Contains(textureName))
        {
          unusedReason = "it does not exist in the GameCube version's textures archive";
          return false;
        }

        if (!DteTextureMap.ContainsKey(textureName))
        {
          unusedReason = "it's known to be unused on GameCube & Wii";
          return false;
        }
    ...
    public static HashSet<string> UnusedTexturesDolphin = new()
  {
    // Unused icons
    "icon_emptyslot",
    "icon_inv_pogostilt",
